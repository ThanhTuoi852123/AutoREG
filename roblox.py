    ...
    while not captcha_creation_status:
        ...
        with thread.session.post(ro_url, json=ro_data) as req:
           ...
           #print(req.text, req.status_code)
            ...
            thread.log("Received: "+captcha_blob, "blue") 
...
async def captchaTask(thread):
    ...
    captcha_payload = {
        ...
        "task": {
            "type": "FunCaptchaTask",
            "proxyType": "http",
            "proxyLogin": solver_proxy[0],
            "proxyPassword": solver_proxy[1].strip("\n"),
            "proxyAddress": solver_proxy[2],
            "proxyPort": int(solver_proxy[3]),
